...
-- percentage of wins =total win/no_of_bids*100
-- each bidder
-- highest to lowest percentage
select * from ipl_bidding_details; -- bidder_id -- distinct bidder -- bin status='win'
select * from ipl_user;
select * from ipl_bidder_points;-- no of bids 
select * from ipl_bidder_details;-- bidder_id,bidder_name
...
FROM
    ipl_user iu
        JOIN
    ipl_bidder_details ibd USING (user_id)
        JOIN
    ipl_bidding_details ibid USING (bidder_id)
        JOIN
    ipl_bidder_points USING (bidder_id) -- to fetch the data no of bids 
GROUP BY bidder_id , bidder_name , no_of_bids
ORDER BY win_percentage DESC;

-- 2.	Display the number of matches conducted at each stadium with the stadium name and city.

-- each stadium -- number of matches 
select * from ipl_stadium; # fetch the name and city of the stadium 
select * from ipl_match_schedule; # fetch stadium id and match id 
select * from ipl_match;
-- partial completed 
select distinct(stadium_id),status from ipl_match_schedule where status='Completed'
order by stadium_id;
-- complete 
-- column name for display the data stadium_id,stadium_name,city,
SELECT 
...
SELECT * FROM ipl_TEAM;
SELECT BID_TEAM,BID_STATUS,COUNT(BID_TEAM) FROM ipl_bidding_details WHERE BID_STATUS='BID';

-- TOTAL BIDS
-- ALONG WITH BID TEAM AND TEAM NAME
...
group by it.team_name with rollup;-- USE FOR SUM OF TOTAL BID PER TEAM 

-- 5.	Show the team id who won the match as per the win details.
select it.team_id,it.remarks winning_team_name,win_details
from ipl_match join ipl_team it on
substr(ipl_match.win_details,6,3)
=substr(it.remarks,1,3)
...
-- or 
SELECT IT.TEAM_NAME,ITP.PLAYER_ROLE,COUNT(ITP.PLAYER_ROLE) NO_OF_ALL_ROUNDERS
FROM ipl_team IT 
JOIN ipl_team_players ITP
USING(TEAM_ID)
...
select PLAYER_ID, PLAYER_ROLE, sum(cast(substring(performance_dtls,position('Wkt-' in performance_dtls)+4,2) as decimal ))
from ipl_team_players;
select * from ipl_team_players;

select performance_dtls,position('Wkt-' in performance_dtls) from ipl_player;
select performance_dtls, substring(performance_dtls,position('Wkt-' in performance_dtls)+4,2) from ipl_player;
select performance_dtls, cast(substring(performance_dtls,position('Wkt-' in performance_dtls)+4,2) as decimal )from ipl_player;
select * from ipl_team;
...
count(case when 
select * from ipl_match; -- toss wins 
select * from ipl_bidder_details;
select * from ipl_bidder_points;
select * from  ipl_match_schedule;
select ibd.bidder_id,ibd.bidder_name,ibp.count(No_of_bids) from ipl_bidder_details ibd join ipl_bidder_points ibp
Group by bidder_id,bidder_name;


-- The view 'winner' created in question 9 is used for this answer
SELECT BIDDER_ID,COUNT(BIDDER_ID),COUNT(*) FROM ipl_bidder_points
GROUP BY BIDDER_ID;
